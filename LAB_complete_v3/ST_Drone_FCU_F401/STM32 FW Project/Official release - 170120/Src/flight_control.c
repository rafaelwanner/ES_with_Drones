...
void FlightControlPID_innerLoop(EulerAngleTypeDef *euler_rc, Gyro_Rad *gyro_rad, AHRS_State_TypeDef *ahrs, P_PI_PIDControlTypeDef *pid, MotorControlTypeDef *motor_pwm, int16_t active, float delta)
{
  ...
  if (active==0x1){
	  /*
	  error = (float) dist->reference - dist->actual;
	  pid_thr_integ += error*pid->ts;
	  if(pid_thr_integ > max_throttle_limit)
		  pid_thr_integ = max_throttle_limit;
	  else if(pid_thr_integ < min_throttle_limit)
		  pid_thr_integ = min_throttle_limit;
	  float delta = pid->thr_kp*error + pid->thr_ki*pid_thr_integ;
	  PRINTF("Error: %d\r\n", (int) error);
	  PRINTF("Actual: %d\r\n",  dist->actual);
	  PRINTF("Reference: %d\r\n", dist->reference);
	  */
  ...
  //motor_thr = ((int16_t) ((THR_GAIN)*(float)gTHR)); 				//ETH parameter
